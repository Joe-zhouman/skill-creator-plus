...
Don't write expectations yet — just the prompts. You'll draft those in Step 3 while the runs are in progress. Validate with `python3 scripts/validate-evals.py tests/workspace/evals.json` before continuing.
...
### Step 3: While runs are in progress, draft expectations

Don't just wait for the runs to finish — you can use this time productively. Draft quantitative expectations for each test case and explain them to the user. If expectations already exist in `tests/workspace/evals.json`, review them and explain what they check.
...
Update the `eval_metadata.json` files and `tests/workspace/evals.json` with the expectations once drafted. Also explain to the user what they'll see in the review — both the qualitative outputs (which you'll walk them through) and the quantitative benchmark.
